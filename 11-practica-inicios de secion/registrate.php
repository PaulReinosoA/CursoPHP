<?php session_start(); 
//aqui agregamos y validamos para registra en la base si view es registrate.view.php


// Comprobamos si ya tiene una sesion
# Si ya tiene una sesion redirigimos al contenido, para que no pueda volver a registrar un usuario.
if (isset($_SESSION['usuario'])) {
	header('Location: index.php');//redirijo si hay sesion iniciada
	die();
}
...
// 	$usuario = stripslashes($usuario);
	...
	if (empty($usuario) or empty($password) or empty($password2)) {
		...
		$statement = $conexion->prepare('SELECT * FROM usuario WHERE uss = :usuario LIMIT 1');
	...
	if ($errores == '') {
		/*INSERT INTO usuario VALUES (0,"Carlos","clopez@outlook.com","mcanencia","12356");*/
		//$statement = $conexion->prepare('INSERT INTO usuarios (id, usuario, pass) VALUES (null, :usuario, :pass)');
		$statement = $conexion->prepare('INSERT INTO usuario VALUES (0,NULL,NULL,:usuario,:pass);');
